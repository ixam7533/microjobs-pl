// components/PublishButtonDirect.tsx - Przycisk publikacji bezpośrednio do płatności
...
interface PublishButtonDirectProps {
  offerData?: any
  onSuccess?: () => void
}

export default function PublishButtonDirect({ offerData, onSuccess }: PublishButtonDirectProps) {
  ...
  useEffect(() => {
    ...
    const checkPurchaseCompletion = () => {
      ...
      if (purchaseCompleted === 'true') {
        ...
        console.log('Zakup Shopify ukończony - odświeżam stronę')
        onSuccess?.()
      }
    }
      ...
      {process.env.NODE_ENV === 'development' && (
        <button
          onClick={() => {
            ...
            alert('Test: Symulacja udanego zakupu')
          }}
          style={{
            position: 'absolute',
            top: '5px',
